...
export function connectMongo (options = {}) {
  return async function (hook) {
    ...
    // client is now an object with connection (MongoClient instance) and db (db object)
    // client = { connection: ..., db: ... }
    client = { connection: await MongoClient.connect(url, _.omit(options, ['hook', 'url', 'dbName', 'clientPath'])) }
...
export function createMongoCollection (options = {}) {
  return async function (hook) {
    ...
    const collections = (await client.db.listCollections({}, { nameOnly: true }).toArray()).map(({ name }) => name)
    let collection = collections.includes(collectionName) ? client.db.collection(collectionName) : null
...
export function dropMongoIndex (options = {}) {
  return async function (hook) {
    ...
    const collectionName = template(item, _.get(options, 'collection', _.snakeCase(item.id)))
    const collections = (await client.db.listCollections({}, { nameOnly: true }).toArray()).map(({ name }) => name)
    const collection = collections.includes(collectionName) ? client.db.collection(collectionName) : null
    if (!collection) {
...
export function createMongoBucket (options = {}) {
  return async function (hook) {
    ...
    const collections = (await client.db.listCollections({}, { nameOnly: true }).toArray()).map(({ name }) => name)
    let bucket = collections.includes(bucketName + '.files') ? client.db.collection(bucketName + '.files') : null
