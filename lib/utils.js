...
export function mergeErrors (errors) {
  ...
  if (error.data?.writeErrors && error.data?.result) {
    ...
    if (error.data.result.toJSON) error.data.result = error.data.result.toJSON()
    errors.forEach((bulkError, index) => {
      // As first error is the base object skip it
      if (index > 0) {
        if (bulkError.data.writeErrors) error.data.writeErrors = error.data.writeErrors.concat(bulkError.data.writeErrors)
        if (bulkError.data.result) {
          if (bulkError.data.result.toJSON) bulkError.data.result = bulkError.data.result.toJSON()
          error.data.result.insertedIds = error.data.result.insertedIds.concat(bulkError.data.result.insertedIds)
          error.data.result.upserted = error.data.result.upserted.concat(bulkError.data.result.upserted)
          error.data.result.nInserted += bulkError.data.result.nInserted
          error.data.result.nMatched += bulkError.data.result.nMatched
          error.data.result.nModified += bulkError.data.result.nModified
          error.data.result.nRemoved += bulkError.data.result.nRemoved
          error.data.result.nUpserted += bulkError.data.result.nUpserted
        }
      }
    })
    // Bulk result also has a copy of errors, remove it to avoid doublons/incoherence
    delete error.data.result.writeErrors
    delete error.data.result.writeConcernErrors
  } else {
