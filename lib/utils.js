...
export function templateQueryObject (item, object, options = {}) {
  ...
  query = convertComparisonOperators(query, options)
  query = convertNumbers(query, options)
  query = convertDates(query, options)
  return query
}
...
export function mergeErrors (errors) {
  ...
  // FIXME: will induce a circular reference, cloning the base error does not seem to work however
  // error.data = errors
  return error
}
