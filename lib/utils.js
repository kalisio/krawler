...
export function mergeErrors (errors) {
  // Handle case when mongodb error has been wrapped by a feathers error.
  // In this case the mongodb error is stored in the error.data field of the feathers error.
  // Put important fields back at the error level.
  errors.forEach((error) => {
    if (error.data?.writeErrors && error.data?.result) {
      error.writeErrors = error.data.writeErrors
      error.result = error.data.result
    }
  })
    ...
    errors.forEach((bulkError, index) => {
      ...
      if (index > 0) {
        ...
        if (bulkError.result) {
          ...
          error.result.allInsertedIds = error.result.allInsertedIds.concat(Object.keys(bulkError.result.insertedIds))
          Object.assign(error.result.insertedIds, bulkError.result.insertedIds)
          error.result.allUpsertedIds = error.result.allUpsertedIds.concat(Object.keys(bulkError.result.upsertedIds))
          Object.assign(error.result.upsertedIds, bulkError.result.upsertedIds)
          error.result.insertedCount += bulkError.result.insertedCount
          error.result.matchedCount += bulkError.result.matchedCount
          error.result.modifiedCount += bulkError.result.modifiedCount
          error.result.deletedCount += bulkError.result.deletedCount
          error.result.upsertedCount += bulkError.result.upsertedCount
        }
      } else {
        // Generate allInsertedIds and allUpsertedIds
        error.result.allInsertedIds = Object.keys(bulkError.result.insertedIds)
        error.result.allUpsertedIds = Object.keys(bulkError.result.upsertedIds)
      }
    })
