...
export function writeCSV (options = {}) {
  return function (hook) {
    ...
    return new Promise(async (resolve, reject) => {
      let store
      try {
        store = await getStoreFromHook(hook, 'writeCSV', options.storePath)
      } catch (error) {
        reject(error)
        return
      }
...
export function mergeCSV (options = {}) {
  return function (hook) {
    ...
    return new Promise(async (resolve, reject) => {
      let store
      try {
        store = await getStoreFromHook(hook, 'mergeCSV', options.storePath)
      } catch (error) {
        reject(error)
        return
      }
...
export function readCSV (options = {}) {
  return function (hook) {
    ...
    return new Promise(async (resolve, reject) => {
      let store
      try {
        store = await getStoreFromHook(hook, 'readCSV', options.storePath)
      } catch (error) {
        reject(error)
        return
      }
