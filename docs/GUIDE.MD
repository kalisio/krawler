...
When using krawler as a Docker container the arguments to the CLI have to be provided through the ARGS environment variable, with the data volume required to make inputs accessible within the container and get output files back:
```
docker pull kalisio/krawler
docker run --name krawler --rm -v /mnt/data:/opt/krawler/data -e "ARGS=./data/jobfile.js" kalisio/krawler
```
